...
fn main() {
    ...
    let temp_cdir = std::env::current_dir().unwrap();
    let cur_dir = &temp_cdir.to_str().unwrap();
    ...
    for v in versions_resp.versions {
        if v.id == "1.14.2" {
            ...
            utils::download_file(libs_resp.asset_index.url, format!("{}/data/profiles/{}/assets/indexes", cur_dir, PROFILE).as_str());

            for (_, asset) in &assets_resp.objects {
            break;
                utils::download_file(format!("{}/{}/{}", RESOURCES, &asset.hash[0..2], &asset.hash), format!("{}/data/profiles/{}/assets/objects/{}", cur_dir, PROFILE, &asset.hash[0..2]).as_str());
            }

            utils::download_file(libs_resp.downloads.client.unwrap().url, format!("{}/data/profiles/{}", cur_dir, PROFILE).as_str());

            for lib in libs_resp.libraries.iter() {
            break;
                match &lib.downloads.artifact {
                    Some(artifact) => utils::download_file(artifact.url.to_owned(), format!("{}/data/profiles/{}/libs", cur_dir, PROFILE).as_str()),
                    None => {}
                }
                ...
                match &lib.downloads.classifiers {
                    Some(classifiers) => {
                        match &classifiers.natives_linux {
                            Some(native) => utils::download_file(native.url.to_owned(), format!("{}/data/profiles/{}/libs", cur_dir, PROFILE).as_str()),
                            None => {}
                        }
                    },
                    None => {}
            ...
            utils::gen_run_cmd(
                format!("{}/data/profiles/{}", cur_dir, PROFILE).as_str(),
                "/usr/bin/java",
